...
def draw_social_dist(
    ...
    for obj in objects:
        ...
        if to.movimiento :
            color_mov = Color.blue
        else :
            color_mov = Color.lime
...
def draw_casco(
    ...
    for obj in objects:
        ...
        id_centroid = centroid(obj.estimate[obj.live_points])
        id_bboxcasco = bboxcasco(obj.estimate[obj.live_points])
        #print(str(id_centroid))
        objectID = obj.id
...
#Dibujo circulo en la cabeza            
        if to.countedcasco is True :
            if to.tienecasco is True:
                colorcasco = (0,255,0)
            if to.tienecasco is False:
                colorcasco = (0,0,255)

            
            #cv2.circle(frame,(id_centroid[0],id_centroid[1]),8,colorcasco,3)
            cv2.rectangle(frame,(id_bboxcasco[0] + 2, id_bboxcasco[2] + 2),(id_bboxcasco[0] + id_bboxcasco[1] - 2, id_bboxcasco[2] + int(id_bboxcasco[3]/2)),color=colorcasco,thickness=line_width )
...
def bboxcasco(tracked_points: np.array) -> Tuple[int, int]:
    num_points = tracked_points.shape[0]
    x = np.sum(tracked_points[0, 0])
    w = np.sum(tracked_points[1, 0]) - x
    y = np.sum(tracked_points[0, 1])
    h = np.sum(tracked_points[1, 1]) - y
    return int(x), int(w),int(y), int(h/3)
